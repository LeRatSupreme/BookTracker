...
import { db } from '../db';
import { ArrowLeft, Save, Trash2, BookOpen, CheckCircle, Clock, Heart, X, Star, Play } from 'lucide-react';
...
const EditBookPage = () => {
    ...
    const handleSessionSave = async (sessionData) => {
        try {
            ...
            const updatedBook = {
                ...
                status: sessionData.endPage >= book.pages ? 'finished' : 'reading',
                finishedDate: sessionData.endPage >= book.pages ? new Date() : book.finishedDate
            };

            await db.books.update(Number(id), updatedBook);
            ...
            alert(`Session enregistrée ! +${sessionData.pagesRead} pages.`);
        } catch (error) {
    ...
    return (
        <div className="pb-48 space-y-6">
            ...
            {/* Content Container - Fixed Layout Bug */}
                    ...
                    <div className="shrink-0 flex flex-col gap-3">
                        {book.cover ? (
                            <div className="relative group">
                                <img src={book.cover} alt="Cover" className="w-32 rounded-lg shadow-xl" />
                                {/* Zen Mode Button - Quick Access */}
                                {(book.status === 'reading' || book.status === 'tbr') && (
                                    <button
                                        onClick={() => setIsSessionOpen(true)}
                                        className="absolute -bottom-4 left-1/2 -translate-x-1/2 bg-brand-600 text-white px-4 py-2 rounded-full shadow-lg flex items-center gap-2 whitespace-nowrap text-sm font-medium hover:scale-105 transition-transform hover:bg-brand-700"
                                    >
                            ...
                            <div className="w-32 h-48 bg-slate-200 dark:bg-slate-700 rounded-lg flex items-center justify-center text-slate-400">
                                No Cover
                            </div>
                        )}
                    </div>
            ...
            {isSessionOpen && (
                <ReadingSession
                    ...
                    onSaveSession={handleSessionSave}
                />
            )}
        </div>
    );
